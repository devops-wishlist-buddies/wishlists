...
class WishlistProduct(db.Model):
  ...
  def create(self):
    logger.info("Creating product with id %s and wishlist with id %s ...",\
  ...
  def update(self):
    logger.info("Updating tuple (product %s and wishlist %s) ...",\
      self.product_id, self.wishlist_id)
    if not self.id:
  ...
  def delete(self):
    logger.info("Deleting tuple (product %s and wishlish %s) ...",\
      self.product_id, self.wishlist_id)
  ...
  def find_all_by_wishlist_id(cls, wishlist_id:int)->list:
    logger.info("WishlistProduct: processing lookup for wishlist_id %s ...",
      wishlist_id)
  ...
  def find_by_wishlist_id_and_product_id(cls, wishlist_id:int,product_id:int):
    logger.info("WishlistProduct: processing lookup for wishlist_id:"\
      "%s and product_id %s ...", wishlist_id,product_id)
  ...
  def delete_all_by_wishlist_id_and_product_id(cls, wishlist_id:int, product_ids:list) -> int:
    logger.info("WishlistProduct: processing deletion for wishlist_id %s"\
